...
class Districts extends CI_Controller {
    ...
    public $table_name = TBL_DISTRICTS;
    public $controllers = 'districts';
    public $view_name = 'districts';
    public $title = 'Districts';
    ...
    function index() {
        ...
        $data['breadcrumb'] = '<!--begin::Item-->
					<li class="breadcrumb-item text-grey-900">Districts</li>
					<!--end::Item-->';        
            ...
            if ($this->form_validation->run()) {
                ...
                if ($id > 0):
                    if ($this->Common->update_info($id, $this->table_name, $post_data, $this->PrimaryKey)):
                        $response = array("status" => "ok", "heading" => "Updated successfully...", "message" => "District updated successfully.");
                    else:
                        $response = array("status" => "error", "heading" => "Not Updated...", "message" => "District not updated successfully.");
                    endif;
                else:
                    if ($this->Common->add_info($this->table_name, $post_data)):
                        $response = array("status" => "ok", "heading" => "Add successfully...", "message" => "District added successfully.");
                    else:
                        $response = array("status" => "error", "heading" => "Not Added successfully...", "message" => "District not added successfully.");
                    endif;
                endif;
            }else {
    ...
    function manage() {
        ...
        $this->datatables->select('d.'.$this->PrimaryKey.', s.name as state_name, d.name,d.short_name,d.rto_code');
        $this->datatables->from($this->table_name.' d')
                ->join(TBL_STATES . ' s', 's.id = d.state_id', 'LEFT')
                ->add_column('action', $this->action_row('$1'), 'd.'.$this->PrimaryKey);
        //$this->datatables->edit_column('Image', $this->show_image('$1'), 'Image');
        $this->datatables->unset_column('d.'.$this->PrimaryKey);
